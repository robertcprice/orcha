...
class HybridOrchestratorV4:
    ...
    def __init__(
        ...
        gpt_model: str = "gpt-4",
        enable_multi_ai_research: bool = True,
        ...
        # Multi-AI research configuration
        self.enable_multi_ai_research = enable_multi_ai_research
        self.research_providers = research_providers or ["openai", "grok", "claude"]

        # Initialize Multi-AI Research
        self.multi_ai_research = None
        if self.enable_multi_ai_research:
            try:
                self.multi_ai_research = MultiAIResearch(
                    openai_api_key=self.openai_key,
                    grok_api_key=os.getenv("GROK_API_KEY") or os.getenv("XAI_API_KEY"),
                    perplexity_api_key=os.getenv("PERPLEXITY_API_KEY"),
                    project_root=self.project_root  # For Claude Code CLI
                )
                available = self.multi_ai_research._get_available_providers()
                print(f"✓ Multi-AI Research enabled with: {', '.join(available)}")
                if "claude" in available:
                    print("  ✓ Claude Code CLI detected (no API key needed)")
            except Exception as e:
                print(f"⚠️  Multi-AI Research initialization failed: {e}")
                self.multi_ai_research = None
...
Respond ONLY with the JSON, no other text."""
    ...
    async def _chatgpt_provide_information(
        ...
        """
        Provides information based on Claude's request.
        Supports:
        - web_search: Actual internet search
        - research: Multi-AI research (OpenAI, Grok, Claude)
        - context/advice/examples: Single AI response
        """

        if verbose:
            print(f"🤖 Processing request: {request.request_type}")

        # Handle research request type with Multi-AI Research
        if request.request_type == "research" and self.multi_ai_research:
            if verbose:
                print(f"🔬 Performing Multi-AI Research: {request.query}")

            try:
                # Use Multi-AI Research
                research_result = await self.multi_ai_research.research_topic(
                    topic=request.query,
                    providers=self.research_providers,
                    synthesize=True,
                    context=json.dumps(request.details) if request.details else None
                )

                if verbose:
                    successful = [r for r in research_result.responses if not r.error]
                    print(f"✓ Research complete: {len(successful)}/{len(research_result.responses)} providers responded")
                    print()

                # Use synthesis if available, otherwise concatenate responses
                content = research_result.synthesis if research_result.synthesis else "\n\n".join([
                    f"**{r.provider}**: {r.content}"
                    for r in research_result.responses if not r.error
                ])

                # Add consensus/divergent points if available
                if research_result.consensus_points:
                    content += "\n\n**Consensus Points:**\n" + "\n".join([
                        f"- {point}" for point in research_result.consensus_points
                    ])

                if research_result.divergent_points:
                    content += "\n\n**Different Perspectives:**\n" + "\n".join([
                        f"- {point}" for point in research_result.divergent_points
                    ])
                ...
                return InformationResponse(
                    ...
                    metadata={
                        "providers": [r.provider for r in research_result.responses if not r.error],
                        "synthesis_performed": research_result.synthesis is not None,
                        "consensus_points": len(research_result.consensus_points),
                        "divergent_points": len(research_result.divergent_points)
                    }
                )

            except Exception as e:
                if verbose:
                    print(f"⚠️  Multi-AI Research failed: {e}, falling back to single AI\n")
