...
export default function Home() {
  ...
  // ✅ FIX: Disabled WebSocket health check to prevent errors when better-sqlite3 is incompatible
  // WebSocket is optional - the system works without it
  // useEffect(() => {
  ...
  const handleTaskSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ NEW: Check if in offline mode and warn user
        if (data.mode === 'offline') {
          console.warn('⚠️ System is in offline simulation mode - API keys missing');
          setRedisErrorMessage(
            'System is in offline mode. Add API keys (DEEPSEEK_API_KEY, GROK_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY) to .env file to enable task execution.'
          );
          setShowRedisError(true);
          setIsTaskActive(false);
