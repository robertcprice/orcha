...
export default function OrchestratorCanvas({ onNodeClick, onActiveNodesChange, onTaskComplete, onLogEvent, onHasSavedState, shouldRestore, onNodeMetadataChange, onRegisterLoadTree, onRegisterStartSequence }: OrchestratorCanvasProps) {
  ...
  const handleWebSocketMessage = useCallback((event: WebSocketEvent) => {
    ...
    // ✅ NEW: Handle agent_spawned events from Python orchestrator
    if (hook_event_type === 'agent_spawned' || eventData.type === 'agent_spawned') {
      const agentRole = eventData.agent || eventData.data?.agent || payload?.agent;
      const agentLabel = eventData.agent_label || eventData.data?.agent_label || payload?.agent_label;
      const metadata = eventData.meta || eventData.data || payload || {};
      const x = metadata.x || 50;
      const y = metadata.y || 50;

      // Map agent role to node ID (e.g., "CLAUDE" -> "planning-claude")
      const nodeId = `planning-${agentRole?.toLowerCase()}`;

      console.log(`🌟 Agent spawned: ${agentRole} at (${x}, ${y})`);

      setAgents(prev => {
        // Create the agent node if it doesn't exist
        if (!prev[nodeId]) {
          return {
            ...prev,
            [nodeId]: {
              id: nodeId,
              name: agentLabel || agentRole,
              type: 'planning',
              status: 'spawning',
              x,
              y,
  ...
  // ✅ FIX: Re-enabled WebSocket connection (SQLite issue resolved with in-memory storage)
