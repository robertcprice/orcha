...
class HybridOrchestratorV4:
    ...
    def __init__(
        ...
        # ✅ CONSTRAINT: Read current project for directory isolation
        ...
        # ✅ CONSTRAINT: Set project output directory
        ...
        # ✅ CONSTRAINT: Pass project output directory to Claude CLI executor
    ...
    async def execute_goal_iterative(
        ...
        # Store the agent activity callback
        ...
        # Spawn ChatGPT agent for planning
        if agent_activity_callback:
            await agent_activity_callback("CHATGPT", "spawn", "Creating comprehensive execution plan")
            await agent_activity_callback("CHATGPT", "status", "thinking", {"task": "Analyzing goal and creating task breakdown"})
            await agent_activity_callback("CHATGPT", "output", f"🤔 ChatGPT thinking: Breaking down '{user_goal[:80]}...' into executable tasks")

        # ChatGPT creates comprehensive execution plan
        execution_plan = await self.chatgpt.create_plan(user_goal, context)

        # ChatGPT outputs plan details
        if agent_activity_callback:
            await agent_activity_callback("CHATGPT", "output", f"📋 Created plan with {len(execution_plan.tasks)} tasks")
            await agent_activity_callback("CHATGPT", "output", f"💡 Reasoning: {execution_plan.reasoning[:200]}...")
            await agent_activity_callback("CHATGPT", "complete", f"Planning complete - {len(execution_plan.tasks)} tasks")
...
Provide the information now:"""
    ...
    async def _claude_execute_stage(
        ...
        # ✅ CONSTRAINT: Get relative path to show in prompt
...
Execute now and respond with JSON:"""
        ...
        if self.agent_activity_callback:
            await self.agent_activity_callback("CLAUDE", "spawn", f"Claude executing turn {turn_number}")
            await self.agent_activity_callback("CLAUDE", "status", "thinking", {"turn": turn_number})
            await self.agent_activity_callback("CLAUDE", "output", f"🤖 Claude thinking: Analyzing execution plan and current state...")

        # Callback to stream Claude output
        async def stream_claude_output(line: str):
            if self.agent_activity_callback and line.strip():
                await self.agent_activity_callback("CLAUDE", "output", f"📝 {line.strip()}")
        ...
        if hasattr(self.claude_cli, 'execute_prompt_streaming'):
            ...
            if self.agent_activity_callback and output:
                lines = output.split('\n')
                for i, line in enumerate(lines[:50]):  # First 50 lines for visibility
                    if line.strip():
                        await self.agent_activity_callback("CLAUDE", "output", f"📝 {line.strip()}")
                if len(lines) > 50:
                    await self.agent_activity_callback("CLAUDE", "output", f"... ({len(lines) - 50} more lines)")

        # Mark completion
        if self.agent_activity_callback:
            await self.agent_activity_callback("CLAUDE", "complete", f"Execution turn {turn_number} complete")
