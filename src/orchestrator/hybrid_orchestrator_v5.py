...
try:
    from orchestrator.superclaude_manager import (
        SuperClaudeManager, BehavioralMode, Persona,
        create_superclaude_manager, apply_superclaude_context
    )
    SUPERCLAUDE_AVAILABLE = True
except ImportError:
    SUPERCLAUDE_AVAILABLE = False
    SuperClaudeManager = None

# Claude-Flow Memory integration
try:
    from orchestrator.claude_flow_memory import ClaudeFlowMemory, SearchResult
    CLAUDEFLOW_MEMORY_AVAILABLE = True
except ImportError:
    CLAUDEFLOW_MEMORY_AVAILABLE = False
    ClaudeFlowMemory = None
...
class HybridOrchestratorV5:
    ...
    def __init__(
        ...
        enable_superclaude: bool = True,
        superclaude_persona: Optional[str] = None,
        superclaude_mode: Optional[str] = None,
        permission_bypass: bool = False,
        # Claude-Flow Memory parameters
        enable_memory: bool = True,
        memory_namespace: str = "orchestration"
    ):
        """
        ...
        Args:
            ...
            enable_superclaude: Enable SuperClaude framework integration
            superclaude_persona: Default SuperClaude persona (e.g., 'architect', 'security')
            superclaude_mode: Default behavioral mode (e.g., 'deep-research', 'token-efficiency')
            permission_bypass: Enable permission bypass for autonomous execution
            enable_memory: Enable Claude-Flow memory integration (96x-164x faster than Redis)
            memory_namespace: Default namespace for memory storage
        """
        self.project_root = project_root
        self.max_iterations = max_iterations
        self.verbose = verbose
        self.agent_activity_callback = agent_activity_callback or self._default_agent_callback
        self.permission_bypass = permission_bypass
        ...
        # SuperClaude integration
        self.superclaude_enabled = enable_superclaude and SUPERCLAUDE_AVAILABLE
        self.superclaude_manager = None
        self.superclaude_persona = superclaude_persona
        self.superclaude_mode = superclaude_mode

        if self.superclaude_enabled:
            try:
                self.superclaude_manager = SuperClaudeManager(str(project_root))
                if self.superclaude_manager.is_superclaude_installed():
                    self._log("✓ SuperClaude Framework loaded")
                    if superclaude_persona:
                        self._log(f"  Persona: {superclaude_persona}")
                    if superclaude_mode:
                        self._log(f"  Mode: {superclaude_mode}")
                    if permission_bypass:
                        self._log("  Permission Bypass: ENABLED")
                else:
                    self._log("⚠ SuperClaude not installed, running without SuperClaude enhancements")
                    self.superclaude_enabled = False
            except Exception as e:
                self._log(f"⚠ SuperClaude initialization failed: {e}")
                self.superclaude_enabled = False

        # Claude-Flow Memory integration
        self.memory_enabled = enable_memory and CLAUDEFLOW_MEMORY_AVAILABLE
        self.memory = None
        self.memory_namespace = memory_namespace

        if self.memory_enabled:
            try:
                self.memory = ClaudeFlowMemory(
                    project_root=project_root,
                    default_namespace=memory_namespace,
                    verbose=verbose
                )
                self._log("✓ Claude-Flow Memory loaded (96x-164x performance boost)")
                self._log(f"  Namespace: {memory_namespace}")
                self._log("  Features: AgentDB vector search + ReasoningBank patterns")
            except Exception as e:
                self._log(f"⚠ Claude-Flow Memory initialization failed: {e}")
    ...
    def _apply_superclaude_context(
        self,
        task: str,
        ...
        """
        Apply SuperClaude context to a task description.
        ...
        Returns:
            Enhanced task description or original if SuperClaude not available
        """
        if not self.superclaude_enabled or not self.superclaude_manager:
            return task

        # Use provided overrides or defaults
        persona_to_use = persona or self.superclaude_persona
        mode_to_use = mode or self.superclaude_mode

        # Convert mode string to enum if needed
        mode_enum = None
        if mode_to_use:
            try:
                mode_enum = BehavioralMode(mode_to_use)
            except ValueError:
                self._log(f"⚠ Invalid SuperClaude mode: {mode_to_use}")

        # Enhance prompt
        try:
            enhanced = self.superclaude_manager.enhance_prompt(
                original_prompt=task,
                persona=persona_to_use,
                mode=mode_enum
            )
            self._log(f"✓ SuperClaude context applied")
            if persona_to_use:
                self._log(f"  Persona: {persona_to_use}")
            if mode_to_use:
                self._log(f"  Mode: {mode_to_use}")
            return enhanced
        except Exception as e:
            self._log(f"⚠ SuperClaude enhancement failed: {e}")
            return task

    def _get_mcp_recommendations(self, task: str) -> List[str]:
        """
        Get MCP server recommendations from SuperClaude.
        ...
        try:
            recommendations = self.superclaude_manager.get_mcp_recommendations(
                task_type=task,
                persona=self.superclaude_persona
            )
            if recommendations:
                self._log(f"✓ MCP Recommendations: {', '.join(recommendations)}")
            return recommendations
        except Exception as e:
            self._log(f"⚠ MCP recommendations failed: {e}")
            return []

    def _validate_with_superclaude(self, response: str) -> Dict[str, Any]:
        """
        Validate response against SuperClaude rules.
        ...
        try:
            return self.superclaude_manager.enforce_rules(response)
        except Exception as e:
            self._log(f"⚠ SuperClaude validation failed: {e}")
            return {'valid': True, 'violations': [], 'warnings': []}
    ...
    async def execute_goal(
        ...
        # Apply SuperClaude context if enabled
        enhanced_user_goal = user_goal
        if self.superclaude_enabled:
            self._log("\n🎭 Applying SuperClaude Framework...")
            enhanced_user_goal = self._apply_superclaude_context(user_goal)

            # Get MCP recommendations
            mcp_recommendations = self._get_mcp_recommendations(user_goal)
            if mcp_recommendations:
                context = context or {}
                context['mcp_recommendations'] = mcp_recommendations
        ...
        if self.memory_enabled and self.memory:
            ...
            try:
                ...
                if similar_tasks:
                    self._log(f"  Found {len(similar_tasks)} similar past workflows")
                    context = context or {}
                    context['similar_past_workflows'] = [
                        {
                            'content': r.content,
                            'similarity': r.similarity_score,
                            'metadata': r.metadata
                        }
                        for r in similar_tasks
                    ]
            except Exception as e:
        ...
        await publish_event({
            ...
            "goal": user_goal,
            "superclaude_enabled": self.superclaude_enabled,
            "superclaude_persona": self.superclaude_persona,
            "superclaude_mode": self.superclaude_mode,
        ...
        try:
            ...
            # Use enhanced goal for planning
            enriched_plan = await self.hybrid_planner.enrich_plan(
                task_title=enhanced_user_goal,
            ...
            # Store workflow result in memory for future learning
            if self.memory_enabled and self.memory:
                self._log("\n💾 Storing workflow result in memory...")
                try:
                    workflow_data = {
                        "session_id": session_id,
                        "goal": user_goal,
                        ...
                        "enrichments_used": len(enriched_plan.enrichments),
                        "workflow_log": workflow_log,
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    }
                    await self.memory.store_workflow_result(session_id, workflow_data)
                    self._log("  ✓ Workflow result stored for future learning")
                except Exception as e:
                    self._log(f"  ⚠ Memory storage failed: {e}")
